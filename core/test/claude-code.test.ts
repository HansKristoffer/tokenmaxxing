...
import { parseClaudeCodeFile } from "../src/sources/claude-code.ts";
import { listClaudeCodeFiles } from "../src/sources/paths.ts";
...
describe("parseClaudeCodeFile (synthetic)", () => {
  it("tags subagent turns with their agentId and keeps the parent sessionId", async () => {
    const path = await makeTempJsonl([
      ...
      JSON.stringify({
        ...
        isSidechain: true,
        agentId: "a4a313cd",
        message: { ...baseAssistant.message, id: "msg-sub" },
      }),
    ]);
    const r = await parseClaudeCodeFile({ path, byteOffset: 0 });
    expect(r.events.map((e) => [e.sessionId, e.agentId])).toEqual([
      ["sess-abc", null],
      ["sess-abc", "a4a313cd"],
    ]);
  });
  ...
  it("advances offset only to last full line and re-reads partial", async () => {
    ...
    const r1 = await parseClaudeCodeFile({ path, byteOffset: 0 });
    ...
    await writeFile(path, `${lineA}\n${lineB}\n`);
    const r2 = await parseClaudeCodeFile({ path, byteOffset: r1.newOffset });
...
describe("parseClaudeCodeFile (real local data)", () => {
  it("parses assistant events from a real session file", async () => {
    ...
    const r = await parseClaudeCodeFile({ path: best.path, byteOffset: 0 });
