...
function readTokenCount(rec: BubbleRecord): { input: number; output: number } {
  ...
  return {
    ...
    output: typeof tc?.outputTokens === "number" && Number.isFinite(tc.outputTokens) ? tc.outputTokens : 0,
  };
}
...
/** Half-open key range equivalent to `LIKE 'prefix%'` (exact case) that the
 *  unique index on `key` can seek — LIKE forces a full scan of the table. */
export function keyPrefixRange(prefix: string): { lo: string; hi: string } {
  const last = prefix.charCodeAt(prefix.length - 1);
  return { lo: prefix, hi: prefix.slice(0, -1) + String.fromCharCode(last + 1) };
}

export const COMPOSER_TIMESTAMPS_SQL = "SELECT key, value FROM cursorDiskKV WHERE key >= $lo AND key < $hi";

export const AGENT_KV_TIMESTAMPS_SQL = `SELECT key, value FROM cursorDiskKV
       WHERE key >= $lo AND key < $hi AND NOT (key >= $blobLo AND key < $blobHi)`;

function loadComposerTimestamps(db: Database): Map<string, ComposerTimestamps> {
  const map = new Map<string, ComposerTimestamps>();
  const { lo, hi } = keyPrefixRange("composerData:");
  const rows = db.query(COMPOSER_TIMESTAMPS_SQL).all({ $lo: lo, $hi: hi }) as Array<{
    key: string;
...
function loadAgentKvTimestamps(db: Database): Map<string, number> {
  ...
  const { lo, hi } = keyPrefixRange("agentKv:");
  const { lo: blobLo, hi: blobHi } = keyPrefixRange("agentKv:blob:");
  const rows = db
    .query(AGENT_KV_TIMESTAMPS_SQL)
    .all({ $lo: lo, $hi: hi, $blobLo: blobLo, $blobHi: blobHi }) as Array<{
    key: string;
    value: string | Uint8Array;
  }>;
...
export function parseCursorLocal(opts: ParseCursorLocalOptions): ParseCursorLocalResult {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const requestId = typeof rec.usageUuid === "string" && rec.usageUuid.length > 0 ? rec.usageUuid : null;
