 ...
 */
export async function parseClaudeCodeFile(opts: ParseClaudeCodeOptions): Promise<ParseClaudeCodeResult> {
  const { path, byteOffset } = opts;
  ...
  for await (const part of readNewlineLines(file, byteOffset)) {
    ...
    const agentId = isString(raw.agentId) ? raw.agentId : null;
    ...
    const cacheCreation = isNum(usage.cache_creation_input_tokens) ? usage.cache_creation_input_tokens : 0;
